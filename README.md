...
4. **Cobrança de clientes** — esperando escolha de provedor de pagamento SaaS. (O deploy em si **já está no ar**, ver "Fechada (2026-09-09): deploy em produção" abaixo.)
...
**Fechada (2026-09-09): deploy em produção (Vercel + Supabase)** — repositório no GitHub (`concilionsistema-collab/mtinvest`); os dois projetos Vercel do time `cionlaris` (já existiam, linkados ao repo, deploy quebrado desde 2026-08-23) voltaram a subir a cada push em `main`. **Portal**: projeto `mtinvest-web` (rootDirectory = raiz do repo, serve `apps/web`), https://mtinvest-web.vercel.app — o nome engana, é o Next.js. **API**: projeto `mtinvest-web-kzbr` (rootDirectory = `apps/api`), https://mtinvest-web-kzbr-one.vercel.app, Cron da Vercel em GET /scheduler/varredura 1x/dia. NEXT_PUBLIC_API_URL do portal ↔ CORS_ORIGIN da API confirmados ponta a ponta (preflight OPTIONS 204; POST /auth/login senha errada → 401 com mensagem de negócio). **Ajuste de plataforma registrado**: deploy do portal falhava DEPOIS do build (`Cannot patch preview comments when immutable static file upload is enabled`) — conflito Next 16.3 immutable static upload × feedback da Vercel; corrigido desligando enablePreviewFeedback/enableProductionFeedback no projeto mtinvest-web via API. Continua manual: migrations e bootstrap de tenant (ver DEPLOY.md).

**Fechada (2026-09-09): `multer` >= 2.3.0 via `overrides`** — `@nestjs/platform-express@11` fixa `multer@2.2.0` (4 advisories de DoS); o upload de foto de perfil passa multipart pelo multer. `overrides.multer` na raiz. `npm audit`: **1 vulnerabilidade** (eram 11 em 2026-09-08). Resta `maplibre-gl` <= 6.4.0 (XSS via `Popup.setHTML`) — não explorável hoje (HTML montado de constantes hardcoded); upgrade 5→6 é major, fica com a revisão visual pendente.

**Fechada (2026-09-09): CI voltou a ficar verde** — de 2026-08-23 a 2026-09-09 a CI falhava a cada push (dois furos de setup pós-`npm ci` que só apareceram quando ela rodou de verdade no GitHub: `prisma generate` nunca rodava e `packages/shared/dist/` não existia). Fechados com um `postinstall` na raiz (`prisma generate` + `build packages/shared`). `actions/checkout`/`setup-node` → `v5`.
...
**Fechada (2026-09-08): upgrade dos frameworks (era dívida técnica de 2026-08-09)** — os dois saltos de major que a nota anterior deixou registrados como pendentes foram aplicados, um framework por vez, com `typecheck` + 291 unit + build completo verdes a cada passo:
- **Next.js 14 → 15 → 16** e **React 18 → 19** (`apps/web`). A app é quase toda client-side (39 de 43 arquivos `'use client'`); a única rota dinâmica (`portal/[token]`) usa o hook `useParams`, não a prop `params` async — então **nenhuma migração de `params`/`searchParams` async** foi necessária. Ajustes: `tsconfig.json` (`jsx: "react-jsx"` + globs de tipo novos), `next.config.js` (`outputFileTracingRoot` pro monorepo; `agentRules: false` porque o Next 16 gera `AGENTS.md`/`CLAUDE.md` em `apps/web` a cada `next dev`), e `JSX.Element` → `ReactElement` importado de `react` em 2 arquivos (o namespace global `JSX` saiu dos tipos do React 19). Build usa Turbopack por padrão no Next 16.
- **NestJS 10 → 11** (Express 5 por baixo, `apps/api`). `@types/express` `^4` → `^5`. **Sem mudança de código**: nenhuma rota usa wildcard/`:param?` (sintaxe de path que mudou no Express 5), `create-app.ts` e `api/index.ts` (serverless) não precisaram de ajuste. `package-lock.json` foi regenerado do zero — os reinstalls parciais tinham deixado cópias aninhadas de `@nestjs/common` (v10 na raiz, v11 em `apps/api`) que quebravam o typecheck com conflito de tipos `DynamicModule`.
- **`npm audit`: 0 vulnerabilidades** (eram 11). Validado também contra o Supabase real via proxy TLS: **11 testes e2e** (JWT → guard → controller → service → RLS por HTTP) e **4 de integração** (isolamento RLS por tenant) passando na stack nova; smoke manual da API: `/health` 200, `ValidationPipe` 400 (`forbidNonWhitelisted`), `JwtAuthGuard` 401.
- **Achado durante o upgrade**: a CI (`ci.yml`) roda `npm ci` → `npm run typecheck` **sem** `prisma generate` — como a CI ainda nunca rodou de verdade (sem `git remote`), a lacuna estava latente (após `npm ci` o `@prisma/client` é só stub e o typecheck de `apps/api` quebra). Corrigido com um `postinstall` na raiz (`prisma generate`), que cobre CI, Vercel e checkout limpo de uma vez.
...
**Fechada (2026-08-09): cabeçalhos de segurança HTTP (`helmet`)** — **extensão registrada**, não especificada em nenhum artefato: recomendação oficial do próprio guia de segurança do NestJS (https://docs.nestjs.com/security/helmet), até agora ausente. Adiciona `Content-Security-Policy`, `X-Content-Type-Options`, `Strict-Transport-Security`, `X-Frame-Options` etc. em toda resposta. **Risco identificado e corrigido antes de fechar**: o default do helmet pra `Cross-Origin-Resource-Policy` é `same-origin`, o que bloquearia o próprio front-end (porta 3000) de ler as respostas da API (porta 3001) — CORP é aplicado pelo navegador independentemente do CORS já configurado em `enableCors`. Configurado explicitamente como `cross-origin`. **Testado de verdade**: contra um processo descartável e, depois, contra o servidor de desenvolvimento real já em execução (`nest start --watch`, que recarregou sozinho) — `curl -D-` confirmando `Cross-Origin-Resource-Policy: cross-origin` e `Access-Control-Allow-Origin: http://localhost:3000` presentes juntos na mesma resposta (`GET /health` com `Origin: http://localhost:3000`), ou seja, os headers novos não quebram o consumo cross-origin que o front-end já faz.
