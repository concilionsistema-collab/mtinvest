...
export default function OportunidadesPage() {
  ...
  return <main className="deals-page"><h1 className="sr-only">Negociações</h1>
    ...
    <div className="deals-primary-grid">
      <section className="deals-surface deals-pipeline"><header className="deals-section-head"><h2>Pipeline de Negociações</h2><div><select aria-label="Tipo do funil"><option>Funil Padrão</option></select><button aria-label="Configurações">⚙</button><button aria-label="Visualização compacta">▦</button><button aria-label="Organizar">⠿</button><button>▽ Filtros⌄</button></div></header><div className="deals-kanban">{stages.map((stage)=><article className={`deals-stage deals-stage--${stage.tone}`} key={stage.title}><header><b>{stage.title}</b><small>{stage.count} negociações</small><strong>{stage.total}</strong></header><div>{stage.cards.map((card,index)=><button className="deals-card" key={card[0]}><Avatar initials={card[4]} index={index}/><span><b>{card[0]}</b><small>{card[1]}</small><strong>{card[2]}</strong><em>{card[3]}</em></span><i>{stage.tone==='green'?'✓':stage.tone==='orange'||stage.tone==='yellow'?'◷':'⌕'}</i></button>)}</div><footer>＋ {stage.more} negociações</footer></article>)}</div></section>
