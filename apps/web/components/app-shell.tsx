...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const novoHref=pathname==='/imoveis'?'/imoveis#novo-imovel':pathname==='/oportunidades'?'/oportunidades#nova-negociacao':pathname==='/unidades'?'/unidades#nova-unidade':pathname==='/visitas'||pathname==='/propostas'?'/oportunidades':'/leads#novo-lead';
  const novoRotulo=pathname==='/imoveis'?'＋ Novo Imóvel':pathname==='/oportunidades'?'＋ Nova Negociação':pathname==='/unidades'?'＋ Nova Unidade':pathname==='/visitas'?'＋ Nova Visita':pathname==='/propostas'?'＋ Nova Proposta':'＋ Novo Lead';
  ...
  return <div className={`app-frame premium-app-frame${menuRecolhido?' app-frame--collapsed':''}`}>
    ...
    <div className="app-main"><header className={`app-header${pathname==='/unidades'?' app-header--units':''}`}><div className="header-page-title"><b>{pathname==='/unidades'?'Unidades da Rede':tituloPagina}</b><small>{pathname==='/'?'Visão geral do negócio':pathname==='/leads'?'Gerencie seus leads e transforme oportunidades em vendas':pathname==='/imoveis'?'Gerencie seu portfólio de imóveis e acompanhe o desempenho de vendas':pathname==='/oportunidades'?'Acompanhe o andamento de todas as negociações':pathname==='/visitas'?'Acompanhe sua agenda, confirmações e resultados das visitas':pathname==='/propostas'?'Acompanhe propostas, contrapropostas e negociações até o fechamento':pathname==='/unidades'?'Gerencie filiais, equipes, carteiras e desempenho da operação.':'Gestão imobiliária'}</small></div>{pathname==='/unidades'?<button type="button" className="head-icon fluent units-header-search" aria-label="Buscar unidades" onClick={()=>document.getElementById('unit-search')?.focus()}>&#xE721;</button>:<div className="search"><span className="fluent">&#xE721;</span><input aria-label="Buscar" placeholder={pathname==='/imoveis'?'Buscar imóveis, leads, proprietários, bairros...':pathname==='/oportunidades'?'Buscar negociações, leads, imóveis ou clientes...':pathname==='/visitas'?'Buscar visitas, clientes ou imóveis...':pathname==='/propostas'?'Buscar por cliente, imóvel ou código...':'Buscar leads, clientes, imóveis, oportunidades...'}/><span className="fluent">&#xE11A;</span></div>}
      ...
      {pathname!=='/unidades'&&<>{pathname==='/propostas'?<Link href="#propostas-filtros" className="new-lead" style={{border:'1px solid var(--border)',background:'var(--bg-card)',color:'var(--text-secondary)',boxShadow:'none'}}><span className="fluent">&#xE71C;</span> Filtros</Link>:<button type="button" onClick={() => setPresentationOpen(true)} className="new-lead" style={{background:'linear-gradient(135deg, var(--purple), #5d35be)',border:'none'}}><span className="fluent">&#xE71C;</span> Apresentação Executiva</button>}<Link href={novoHref} className="new-lead">{novoRotulo}</Link>{pathname!=='/propostas'&&<TopThemeSelector />}</>}
      <div className="header-actions" ref={headerActionsRef}>
        {pathname==='/propostas'&&<button type="button" className={`head-icon fluent${menuCabecalho==='agenda'?' head-icon--active':''}`} aria-label="Abrir agenda" aria-haspopup="menu" aria-expanded={menuCabecalho==='agenda'} onClick={()=>setMenuCabecalho((atual)=>atual==='agenda'?null:'agenda')}>&#xE787;</button>}
        ...
        {pathname!=='/unidades'&&<>{pathname!=='/propostas'&&<button type="button" className={`head-icon fluent${menuCabecalho==='agenda'?' head-icon--active':''}`} aria-label="Abrir agenda" aria-haspopup="menu" aria-expanded={menuCabecalho==='agenda'} onClick={()=>setMenuCabecalho((atual)=>atual==='agenda'?null:'agenda')}>&#xE787;</button>}<button type="button" className={`head-icon fluent${menuCabecalho==='mensagens'?' head-icon--active':''}`} aria-label="Abrir mensagens" aria-haspopup="menu" aria-expanded={menuCabecalho==='mensagens'} onClick={()=>setMenuCabecalho((atual)=>atual==='mensagens'?null:'mensagens')}>&#xE8BD;</button></>}
