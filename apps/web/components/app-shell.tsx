...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-frame premium-app-frame${menuRecolhido?' app-frame--collapsed':''}`}>
    ...
    <div className="app-main"><header className={`app-header${pathname==='/unidades'?' app-header--units':''}`}><div className="header-page-title"><b>{pathname==='/unidades'?'Unidades da Rede':tituloPagina}</b><small>{pathname==='/'?'Visão geral do negócio':pathname==='/leads'?'Gerencie seus leads e transforme oportunidades em vendas':pathname==='/imoveis'?'Gerencie seu portfólio de imóveis e acompanhe o desempenho de vendas':pathname==='/oportunidades'?'Acompanhe o andamento de todas as negociações':pathname==='/visitas'?'Acompanhe sua agenda, confirmações e resultados das visitas':pathname==='/propostas'?'Acompanhe propostas, contrapropostas e negociações até o fechamento':pathname==='/contratos'?'Gerencie contratos, documentação e etapas até o fechamento':pathname==='/carteiras'?'Organize e acompanhe a transferência de carteiras entre unidades e corretores':pathname==='/equipe'?'Cadastre, gerencie e acompanhe corretores, gestores e sócios da unidade':pathname==='/marketing'?'Acompanhe o desempenho dos seus canais e gerencie campanhas e leads':pathname==='/relatorios'?'Acompanhe indicadores, funis e resultados em tempo real':pathname==='/financeiro'?'Acompanhe VGV, comissões e resultados do seu negócio':pathname==='/unidades'?'Gerencie filiais, equipes, carteiras e desempenho da operação.':'Gestão imobiliária'}</small></div>{pathname==='/unidades'?<button type="button" className="head-icon fluent units-header-search" aria-label="Buscar unidades" onClick={()=>document.getElementById('unit-search')?.focus()}>&#xE721;</button>:<div className="search"><span className="fluent">&#xE721;</span><input aria-label="Buscar" placeholder={pathname==='/imoveis'?'Buscar imóveis, leads, proprietários, bairros...':pathname==='/oportunidades'?'Buscar negociações, leads, imóveis ou clientes...':pathname==='/visitas'?'Buscar visitas, clientes ou imóveis...':pathname==='/propostas'?'Buscar por cliente, imóvel ou código...':pathname==='/contratos'?'Buscar por cliente, imóvel ou nº do contrato...':pathname==='/carteiras'?'Buscar por lead, corretor, unidade ou carteira...':pathname==='/equipe'?'Buscar por nome, e-mail ou função...':pathname==='/marketing'?'Buscar por canal, campanha ou lead...':pathname==='/relatorios'?'Buscar relatório, indicador ou métrica...':pathname==='/financeiro'?'Buscar por período, corretor ou contrato...':'Buscar leads, clientes, imóveis, oportunidades...'}/><span className="fluent">&#xE11A;</span></div>}
      ...
      {pathname!=='/unidades'&&<>{pathname==='/propostas'||pathname==='/contratos'||pathname==='/carteiras'||pathname==='/equipe'||pathname==='/marketing'||pathname==='/relatorios'||pathname==='/financeiro'?<Link href={pathname==='/propostas'?'#propostas-filtros':pathname==='/contratos'?'#contratos-filtros':pathname==='/carteiras'?'#carteiras-filtros':pathname==='/equipe'?'#equipe-filtros':pathname==='/marketing'?'#marketing-filtros':pathname==='/relatorios'?'#relatorios-disponiveis':'#financeiro-filtros'} className="new-lead" style={{border:'1px solid var(--border)',background:'var(--bg-card)',color:'var(--text-secondary)',boxShadow:'none'}}><span className="fluent">&#xE71C;</span> Filtros</Link>:<button type="button" onClick={() => setPresentationOpen(true)} className="new-lead" style={{background:'linear-gradient(135deg, var(--purple), #5d35be)',border:'none'}}><span className="fluent">&#xE71C;</span> Apresentação Executiva</button>}<Link href={novoHref} className="new-lead">{novoRotulo}</Link>{pathname!=='/propostas'&&pathname!=='/contratos'&&pathname!=='/carteiras'&&pathname!=='/equipe'&&pathname!=='/marketing'&&pathname!=='/relatorios'&&pathname!=='/financeiro'&&<TopThemeSelector />}</>}
      <div className="header-actions" ref={headerActionsRef}>
        {(pathname==='/propostas'||pathname==='/contratos'||pathname==='/carteiras'||pathname==='/equipe'||pathname==='/marketing'||pathname==='/relatorios'||pathname==='/financeiro')&&<button type="button" className={`head-icon fluent${menuCabecalho==='agenda'?' head-icon--active':''}`} aria-label="Abrir agenda" aria-haspopup="menu" aria-expanded={menuCabecalho==='agenda'} onClick={()=>setMenuCabecalho((atual)=>atual==='agenda'?null:'agenda')}>&#xE787;</button>}
        ...
        {pathname!=='/unidades'&&<>{pathname!=='/propostas'&&pathname!=='/contratos'&&pathname!=='/carteiras'&&pathname!=='/equipe'&&pathname!=='/marketing'&&pathname!=='/relatorios'&&pathname!=='/financeiro'&&<button type="button" className={`head-icon fluent${menuCabecalho==='agenda'?' head-icon--active':''}`} aria-label="Abrir agenda" aria-haspopup="menu" aria-expanded={menuCabecalho==='agenda'} onClick={()=>setMenuCabecalho((atual)=>atual==='agenda'?null:'agenda')}>&#xE787;</button>}<button type="button" className={`head-icon fluent${menuCabecalho==='mensagens'?' head-icon--active':''}`} aria-label="Abrir mensagens" aria-haspopup="menu" aria-expanded={menuCabecalho==='mensagens'} onClick={()=>setMenuCabecalho((atual)=>atual==='mensagens'?null:'mensagens')}>&#xE8BD;</button></>}
