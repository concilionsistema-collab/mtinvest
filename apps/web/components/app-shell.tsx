...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const tituloPagina=nav.find((item)=>item.href===pathname)?.label ?? 'Concilion CRM';
  ...
  return <div className={`app-frame premium-app-frame${menuRecolhido?' app-frame--collapsed':''}`}>
    <aside className="app-sidebar premium-sidebar">
      <div className="brand tenant-brand"><div className="tenant-brand__identity" aria-label="Concilion CRM"><span className="tenant-brand__mark" aria-hidden="true">C</span><span className="tenant-brand__copy"><b>Concilion</b><small>CRM</small></span></div><div className="tenant-brand__organization"><img src="/mt-invest-shield.png" alt="" aria-hidden="true"/><span><small>Organização</small><b>MT Invest</b></span></div><button type="button" onClick={()=>setMenuRecolhido((atual)=>!atual)} aria-label={menuRecolhido?'Expandir menu':'Recolher menu'} aria-expanded={!menuRecolhido}>☰</button></div>
      ...
      <div className="mobile-card"><div className="phone-mock"><span>⌂</span><span>◫</span><span>♙</span><span>▦</span></div><div><b>Concilion CRM</b><small>Mobile</small><p>Gerencie seu negócio<br/>de onde estiver.</p><em> App Store</em><em>▶ Google Play</em></div></div>
      ...
      <div className="system-creator-brand"><small>Plataforma de gestão imobiliária</small><b>Concilion CRM</b></div>
    </aside>
